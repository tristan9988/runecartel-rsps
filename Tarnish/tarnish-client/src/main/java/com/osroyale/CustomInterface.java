...
//        personalStores(textDrawingAreas);
    ...
    private static void bankVault(TextDrawingArea[] tda) {
        ...
        addSprite(56301, 862);
        addText(56302, "RuneCartel's Bank Vault", tda, 2, 0xFFA500, true, true);
    ...
    private static void achievementInterface(TextDrawingArea[] tda) {
        ...
        addSprite(56801, 861);
        addText(56802, "RuneCartel's Achievements", tda, 2, 0xFFA500, true, true);
    ...
    private static void welcome(TextDrawingArea[] TDA) {
        ...
        addSprite(21300, 778);
        addHoverButton(21301, 298, 367, 45, "Enter RuneCartel's HQ", -1, 21302, 1);
        ...
        addText(21315, "RuneCartel's HQ welcomes you!", 0x000000, true, false, 52, TDA, 0);
        addText(21316, "RuneCartel's HQ welcomes you!", 0x000000, true, false, 52, TDA, 0);
        addText(21317, "RuneCartel's HQ welcomes you!", 0x000000, true, false, 52, TDA, 0);
...
//            addDropdownMenu(26811, 136, 0, false, true, Dropdown.DEFAULT, "10", "100", "1,000", "10,000", "100,000");
  ...
  //          addDropdownMenu(26811, 150, -1, false, false, Dropdown.KEYBINDING, "None", "F1", "F2", "F3", "F4", "F5", "F6", "F7", "F8", "F9", "F10", "F11", "F12", "ESC");
    ...
    private static void starter(TextDrawingArea[] daniel) {
        ...
        addSprite(45001, 275);
        addText(45002, "RuneCartel's Account Setup", 0xff9933, true, true, 52, daniel, 2);
    ...
    private static void bank(TextDrawingArea[] tda) {
        ...
        addText(interfaceId + 5, "The Bank of RuneCartel", tda, 2, 0xFF981F, true, true);
...
//        bank.child(60, interfaceId + 78, 19, 240);
    ...
    private static void informationTab(TextDrawingArea tda[]) {
        ...
        addHoverText(29404, "www.runecartel.com", "Visit website", tda, 3, 0xC47423, true, true, 168);
    ...
    private static void toolPanel(TextDrawingArea tda[]) {
        ...
        addHoverText(35414, "www.runecartel.com", "Visit website", tda, 3, 0xC47423, true, true, 168);
    ...
    /**
     * Spawn tab interface - sidebar tab with item spawns and gear loadout presets.
     * Interface ID: 42500
     * Scroll area: 42510
     * Buttons: 42520 - 42599
     */
    private static void spawnTab(TextDrawingArea[] tda) {
        RSInterface mainTab = addTabInterface(42500);

        // Title
        addText(42501, "<col=FF9900>Spawn Menu", tda, 2, 0xFF9900, true, true);

        // --- Scroll area ---
        RSInterface scroll = addTabInterface(42510);
        scroll.scrollPosition = 0;
        scroll.contentType = 0;
        scroll.width = 174;
        scroll.height = 230;
        scroll.scrollMax = 720;

        // We'll build clickable text lines inside the scroll
        int id = 42520;
        int childCount = 0;

        // === FOOD ===
        // 42520 = header "--- Food ---"
        addText(id, "--- Food ---", tda, 1, 0xFFCC00, true, true); childCount++; id++;
        // 42521 = Shark x10
        addHoverText(id, "Shark x10", "Spawn Shark x10", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42522 = Manta Ray x10
        addHoverText(id, "Manta Ray x10", "Spawn Manta Ray x10", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42523 = Karambwan x10
        addHoverText(id, "Karambwan x10", "Spawn Karambwan x10", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42524 = Anglerfish x10
        addHoverText(id, "Anglerfish x10", "Spawn Anglerfish x10", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42525 = Dark Crab x10
        addHoverText(id, "Dark Crab x10", "Spawn Dark Crab x10", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42526 = Lobster x10
        addHoverText(id, "Lobster x10", "Spawn Lobster x10", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;

        // === POTIONS ===
        // 42527 = header
        addText(id, "--- Potions ---", tda, 1, 0xFFCC00, true, true); childCount++; id++;
        // 42528 = Super Combat (4)
        addHoverText(id, "Super Combat (4)", "Spawn Super Combat (4)", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42529 = Super Restore (4)
        addHoverText(id, "Super Restore (4)", "Spawn Super Restore (4)", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42530 = Saradomin Brew (4)
        addHoverText(id, "Saradomin Brew (4)", "Spawn Saradomin Brew (4)", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42531 = Ranging Potion (4)
        addHoverText(id, "Ranging Potion (4)", "Spawn Ranging Potion (4)", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42532 = Antifire (4)
        addHoverText(id, "Antifire (4)", "Spawn Antifire (4)", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42533 = Stamina (4)
        addHoverText(id, "Stamina Potion (4)", "Spawn Stamina (4)", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;

        // === RUNES ===
        // 42534 = header
        addText(id, "--- Runes ---", tda, 1, 0xFFCC00, true, true); childCount++; id++;
        // 42535 = Death Rune x500
        addHoverText(id, "Death Rune x500", "Spawn Death Rune x500", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42536 = Blood Rune x500
        addHoverText(id, "Blood Rune x500", "Spawn Blood Rune x500", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42537 = Soul Rune x500
        addHoverText(id, "Soul Rune x500", "Spawn Soul Rune x500", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42538 = Water Rune x500
        addHoverText(id, "Water Rune x500", "Spawn Water Rune x500", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42539 = Fire Rune x500
        addHoverText(id, "Fire Rune x500", "Spawn Fire Rune x500", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42540 = Air Rune x500
        addHoverText(id, "Air Rune x500", "Spawn Air Rune x500", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42541 = Astral Rune x500
        addHoverText(id, "Astral Rune x500", "Spawn Astral Rune x500", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;

        // === BASIC WEAPONS ===
        // 42542 = header
        addText(id, "--- Weapons ---", tda, 1, 0xFFCC00, true, true); childCount++; id++;
        // 42543 = Dragon Scimitar
        addHoverText(id, "Dragon Scimitar", "Spawn Dragon Scimitar", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42544 = Dragon Dagger (p++)
        addHoverText(id, "Dragon Dagger (p++)", "Spawn Dragon Dagger (p++)", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42545 = Rune Crossbow
        addHoverText(id, "Rune Crossbow", "Spawn Rune Crossbow", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42546 = Magic Shortbow (i)
        addHoverText(id, "Magic Shortbow (i)", "Spawn Magic Shortbow (i)", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42547 = Granite Maul
        addHoverText(id, "Granite Maul", "Spawn Granite Maul", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42548 = Abyssal Whip
        addHoverText(id, "Abyssal Whip", "Spawn Abyssal Whip", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;

        // === BASIC ARMOUR ===
        // 42549 = header
        addText(id, "--- Armour ---", tda, 1, 0xFFCC00, true, true); childCount++; id++;
        // 42550 = Rune Full Helm
        addHoverText(id, "Rune Full Helm", "Spawn Rune Full Helm", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42551 = Rune Platebody
        addHoverText(id, "Rune Platebody", "Spawn Rune Platebody", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42552 = Rune Platelegs
        addHoverText(id, "Rune Platelegs", "Spawn Rune Platelegs", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42553 = Rune Kiteshield
        addHoverText(id, "Rune Kiteshield", "Spawn Rune Kiteshield", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42554 = Green D'hide Body
        addHoverText(id, "Green D'hide Body", "Spawn Green D'hide Body", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42555 = Green D'hide Chaps
        addHoverText(id, "Green D'hide Chaps", "Spawn Green D'hide Chaps", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42556 = Amulet of Glory
        addHoverText(id, "Amulet of Glory", "Spawn Amulet of Glory", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42557 = Ring of Recoil
        addHoverText(id, "Ring of Recoil", "Spawn Ring of Recoil", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;

        // === AMMO ===
        // 42558 = header
        addText(id, "--- Ammo ---", tda, 1, 0xFFCC00, true, true); childCount++; id++;
        // 42559 = Rune Arrow x200
        addHoverText(id, "Rune Arrow x200", "Spawn Rune Arrow x200", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42560 = Diamond Bolts (e) x200
        addHoverText(id, "Diamond Bolts (e) x200", "Spawn Diamond Bolts (e) x200", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42561 = Ruby Bolts (e) x200
        addHoverText(id, "Ruby Bolts (e) x200", "Spawn Ruby Bolts (e) x200", tda, 0, 0xD46E08, 0xFFFFFF, false, true, 168); childCount++; id++;

        // === GEAR LOADOUT PRESETS ===
        // 42562 = header
        addText(id, "--- Gear Loadouts ---", tda, 1, 0xFFCC00, true, true); childCount++; id++;
        // 42563 = AGS Rune Melee
        addHoverText(id, "<col=00FF80>AGS Rune Melee", "Equip AGS Rune Melee loadout", tda, 1, 0x00FF80, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42564 = Welfare Rune Melee
        addHoverText(id, "<col=00FF80>Welfare Rune Melee", "Equip Welfare Rune Melee loadout", tda, 1, 0x00FF80, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42565 = Pure Melee
        addHoverText(id, "<col=00FF80>Pure Melee", "Equip Pure Melee loadout", tda, 1, 0x00FF80, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42566 = Pure Range Melee
        addHoverText(id, "<col=00FF80>Pure Range Melee", "Equip Pure Range Melee loadout", tda, 1, 0x00FF80, 0xFFFFFF, false, true, 168); childCount++; id++;
        // 42567 = Zerker Melee
        addHoverText(id, "<col=00FF80>Zerker Melee", "Equip Zerker Melee loadout", tda, 1, 0x00FF80, 0xFFFFFF, false, true, 168); childCount++; id++;

        // Now place all children in the scroll
        scroll.totalChildren(childCount);
        int y = 5;
        int child = 0;
        for (int i = 42520; i < 42520 + childCount; i++) {
            scroll.child(child, i, 5, y);
            child++;
            y += 18;
        }

        // Main tab layout: title + scroll
        mainTab.totalChildren(2);
        mainTab.child(0, 42501, 93, 3);
        mainTab.child(1, 42510, 0, 22);
    }
}
