...
public final class World {
    ...
    public static void update(int ticks) {
        if (!update.get()) {
            update.set(true);
            schedule(new SystemUpdateEvent(ticks));
            // Client expects time in client ticks (50 per second)
            // Server ticks are 600ms each, so: clientTicks = serverTicks * 600 / 20 = serverTicks * 30
            int clientTicks = ticks * 30;
            get().players.stream().forEach(it -> it.send(new SendGameMessage(0, clientTicks, "System update" + " in:")));
        }
    }
    ...
    public static void shutdown() {
        ...
        // Launch auto-update-restart script to rebuild and restart the server
        try {
            String workingDir = System.getProperty("user.dir");
            String script = workingDir + java.io.File.separator + "auto-update-restart.bat";
            java.io.File scriptFile = new java.io.File(script);
            if (scriptFile.exists()) {
                System.out.println("Launching auto-update-restart...");
                new ProcessBuilder("cmd.exe", "/c", "start", "Tarnish Auto-Restart", script)
                        .directory(new java.io.File(workingDir))
                        .inheritIO()
                        .start();
            } else {
                System.out.println("auto-update-restart.bat not found at: " + script);
            }
        } catch (Exception e) {
            System.out.println("Failed to launch auto-update-restart: " + e.getMessage());
        }
    ...
    public static void sendBroadcast(int seconds, String message, boolean countdown) {
        // Convert seconds to client ticks (50 ticks per second)
        int clientTicks = seconds * 50;
        get().players.stream().forEach($it -> {
            $it.send(new SendGameMessage(countdown ? 0 : 1, clientTicks, Utility.capitalizeSentence(message)));
