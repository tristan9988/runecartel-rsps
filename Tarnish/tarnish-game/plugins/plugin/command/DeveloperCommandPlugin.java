        ...
        commands.add(new Command("hp") {
            ...
            public void execute(Player player, CommandParser parser) {
                if (parser.hasNext()) {
                    ...
                    player.skills.setLevel(Skill.HITPOINTS, finalAmount);
                    player.skills.get(Skill.HITPOINTS).setMaxLevel(finalAmount);
                    player.skills.refresh(Skill.HITPOINTS);
                    player.send(new SendMessage("<col=ff0000>Your hitpoints have been set to " + finalAmount + "."));
                } else {
                    player.send(new SendMessage("Usage: ::hp amount (e.g. ::hp 99)"));
                }
            }
        });
